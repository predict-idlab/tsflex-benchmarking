...
f_sum_mean = FuncWrapper(
    sum_mean, output_names=["area", "mean"], vectorized=True, axis=1
)

# (For convenience) we store the constructed `NumpyFuncWrappers` in a list
segment_funcs = [
    FuncWrapper(f, vectorized=True, axis=1)
    for f in [
        np.min,
        np.max,
    ]
...
if args.multiprocessing:
    with VizTracer(
        log_gc=False,
        log_async=True,
        output_file=f"{args.save_dir}/tsflex_vect_mp{'' if args.numb_window_strides == 1 else '_numb_ws=' + str(args.numb_window_strides)}_{args.index}.json",
        max_stack_depth=0,
    ...
    with VizTracer(
        log_gc=False,
        log_async=True,
        output_file=f"{args.save_dir}/tsflex_vect_sequential{'' if args.numb_window_strides == 1 else '_numb_ws=' + str(args.numb_window_strides)}_{args.index}.json",
        max_stack_depth=0,
        plugins=["vizplugins.cpu_usage", "vizplugins.memory_usage"],
    ):
        fc = FeatureCollection(
